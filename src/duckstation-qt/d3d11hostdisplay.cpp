...
QtDisplayWidget* D3D11HostDisplay::createWidget(QWidget* parent)
{
  ...
  if (!createSwapChain())
    Panic("Failed to create swap chain for new widget");

  return widget;
}

void D3D11HostDisplay::destroyWidget()
{
  m_swap_chain_rtv.Reset();
  m_swap_chain.Reset();
  QtHostDisplay::destroyWidget();
}
...
bool D3D11HostDisplay::createDeviceContext(QThread* worker_thread, bool debug_device)
{
  UINT create_flags = 0;
  if (debug_device)
  ...
  if (!QtHostDisplay::createDeviceContext(worker_thread, debug_device))
  {
    m_swap_chain.Reset();
    m_context.Reset();
    m_device.Reset();
...
bool D3D11HostDisplay::createSwapChain()
{
  ...
  swap_chain_desc.OutputWindow = reinterpret_cast<HWND>(m_widget->winId());
...
void D3D11HostDisplay::recreateSwapChain()
{
  m_swap_chain_rtv.Reset();
  m_swap_chain.Reset();
