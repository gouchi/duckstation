...
void MainWindow::setupAdditionalUi()
{
  ...
  for (u32 i = 0; i < static_cast<u32>(DisplayCropMode::Count); i++)
  {
    ...
    connect(action, &QAction::triggered, [this, crop_mode]() {
      m_host_interface->putSettingValue(QStringLiteral("Display/CropMode"),
                                        QString::fromUtf8(Settings::GetDisplayCropModeName(crop_mode)));
      m_host_interface->applySettings();
...
void MainWindow::connectSignals()
{
  ...
  SettingWidgetBinder::BindWidgetToBoolSetting(m_host_interface, m_ui.actionDisableInterlacing,
                                               "GPU/DisableInterlacing");
  SettingWidgetBinder::BindWidgetToBoolSetting(m_host_interface, m_ui.actionForceNTSCTimings,
...
{
  std::optional<DisplayCropMode> current_crop_mode = Settings::ParseDisplayCropMode(
    m_host_interface->getSettingValue("Display/CropMode").toString().toUtf8().constData());
